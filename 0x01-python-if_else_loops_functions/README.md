...
OAOAOA  * [8-uppercase.py](./8-uppercase.py): Python function that prints a string in
OAOAOAOAOAOA  uppercase followed by a new line.
OAOAOA  * Using no more than two `print` functions and one loop.
  * Without importing modules or using `str.upper()` or `str.isupper()`.
